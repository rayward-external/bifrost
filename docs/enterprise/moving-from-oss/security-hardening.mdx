...
description: "Mandatory controls before exposing Bifrost Enterprise to production traffic: SCIM-enforced identity, scoped virtual keys, locked-down CORS, and a minimal header allowlist."
icon: "shield-halved"
---
...
## 1. SCIM-provisioned identity is enforced

User access to the Enterprise dashboard and admin APIs is gated by SCIM provisioning from your identity provider. Manual user creation and ad-hoc invites are not the supported path once your IdP is wired up.
...
- Enable [User Provisioning (SCIM)](/enterprise/user-provisioning) so lifecycle events (joiner / mover / leaver) propagate automatically.
...
<Steps>
<Step title="SCIM-only user provisioning">
