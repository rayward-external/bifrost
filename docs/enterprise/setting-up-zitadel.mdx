...
2. Select **Zitadel** as the OIDC Provider.
...
Bifrost does not currently support Zitadel SCIM management APIs or inbound Zitadel SCIM provisioning. Do not configure a Zitadel SCIM connector, webhook worker, or automation layer to call Bifrost `/scim/v2` yet.

For Zitadel, Bifrost uses the service account setup above to sync users in the background every **24 hours**. During that sync, Bifrost reconciles imported users, role mappings, team mappings, and business-unit mappings from Zitadel.

Every **15 minutes**, Bifrost also refreshes active OIDC sessions. If a session cannot be refreshed, Bifrost checks with Zitadel whether the user is still active; if Zitadel reports the user is inactive, Bifrost decommissions that user locally.

Support for Zitadel SCIM management APIs is coming soon.
