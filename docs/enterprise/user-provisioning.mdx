...
| **Access profile mapping** | Map claim values directly to access profiles via `attributeAccessProfileMappings`, bypassing the default role-derived assignment. First matching rule wins (a user holds a single access profile). |
...
| Access profile mappings | `attributeAccessProfileMappings` | No | **Plain only** | Array of `{ attribute, value, accessProfile }` objects (first match wins) |
...
- **Access profile mappings** — first match wins, like role mappings. A user holds a single access profile, so only the first matching rule (in declaration order) whose profile exists is applied.
