...
// The full migration chain leaves the managed-batch owner keyset index in place
...
func TestMigrationAddManagedBatchOwnerKeysetIndex(t *testing.T) {
	const idx = "idx_managed_batch_owner_keyset"

	t.Run("present after full fresh migration chain", func(t *testing.T) {
		_, db := setupFullMigrationDB(t)
		assert.True(t, db.Migrator().HasIndex(&tables.TableManagedBatch{}, idx),
	...
	t.Run("backfills a pre-existing table missing the index, idempotently", func(t *testing.T) {
		...
		require.NoError(t, db.AutoMigrate(&tables.TableManagedBatch{}))
		mg := db.Migrator()
		if mg.HasIndex(&tables.TableManagedBatch{}, idx) {
			require.NoError(t, mg.DropIndex(&tables.TableManagedBatch{}, idx))
		}
		require.False(t, mg.HasIndex(&tables.TableManagedBatch{}, idx), "precondition: index absent")

		// Backfill migration creates it.
		require.NoError(t, migrationAddManagedBatchOwnerKeysetIndex(ctx, db, testMigrationLogger))
		assert.True(t, mg.HasIndex(&tables.TableManagedBatch{}, idx), "migration must create the keyset index")

		// Running it again is a no-op, not an error.
		require.NoError(t, migrationAddManagedBatchOwnerKeysetIndex(ctx, db, testMigrationLogger))
		assert.True(t, mg.HasIndex(&tables.TableManagedBatch{}, idx))
	})
}
