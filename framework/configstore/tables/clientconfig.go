...
type TableClientConfig struct {
	...
	UnknownBatchIDPolicy                  string                         `gorm:"column:unknown_batch_id_policy;type:varchar(20)" json:"unknown_batch_id_policy,omitempty"` // Governance batch-ownership policy for ledger-absent batch ids: deny|allow (empty = allow)
	BatchAdminVirtualKeyIDsJSON           string                         `gorm:"type:text" json:"-"`                                                                       // JSON serialized []string of batch-admin virtual key IDs
	...
	BatchAdminVirtualKeyIDs []string                  `gorm:"-" json:"batch_admin_virtual_key_ids,omitempty"`
...
func (cc *TableClientConfig) BeforeSave(tx *gorm.DB) error {
	...
	if cc.BatchAdminVirtualKeyIDs != nil {
		data, err := json.Marshal(cc.BatchAdminVirtualKeyIDs)
		if err != nil {
			return err
		}
		cc.BatchAdminVirtualKeyIDsJSON = string(data)
	} else {
		cc.BatchAdminVirtualKeyIDsJSON = "[]"
	}
...
func (cc *TableClientConfig) AfterFind(tx *gorm.DB) error {
	...
	if cc.BatchAdminVirtualKeyIDsJSON != "" {
		if err := json.Unmarshal([]byte(cc.BatchAdminVirtualKeyIDsJSON), &cc.BatchAdminVirtualKeyIDs); err != nil {
			return err
		}
	}
