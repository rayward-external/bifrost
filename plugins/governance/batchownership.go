...
//   - LIST    (PostLLMHook): filter the SINGLE upstream page the request already
//     fetched to the caller's ledger-known rows, set first_id/last_id to the
//     caller's own ids, DROP the pagination cursor entirely, and clear the raw
//     passthrough so every dialect converter rebuilds from the filtered Data.
//     Leak-proof by construction: no returned field can carry or encode a batch
//     id the caller does not own. (Deferred limitation below.)
...
//
// DEFERRED LIMITATION (deep owner-scoped list pagination): LIST returns the
// caller's batches from the most recent upstream page only; owned batches older
// than that page are reachable by retrieve-by-id but are not paginated through in
// list output. Deep multi-page owner pagination was intentionally deferred (it
// required cross-provider cursor rewriting whose leak surface outweighed the
// benefit for a cold secondary gateway); tracked as a follow-up.
//
...
// enforcement needs to issue its one in-process sub-request: a compensating
// cancel after a failed ownership capture. *bifrost.Bifrost satisfies it. It is
// injected post-construction (SetBatchLifecycleClient) because the core client is
// created after the plugins, so it may be nil (the cancel degrades — see call site).
...
// applyBatchOwnershipPostHook records ownership on a successful create and
// scopes list output to the caller. Returns the (possibly mutated) result and a
// non-nil BifrostError only when a create-time ledger write fails (which must
// fail the request). Never errors for list.
...
// filterBatchListForCaller narrows the SINGLE upstream list page the request
// already fetched to the caller's ledger-known rows, sets first_id/last_id to the
// caller's own ids, DROPS the pagination cursor entirely, and suppresses the raw
// passthrough so the dialect converter rebuilds from the filtered Data. Admin keys
// are exempt (full list, raw preserved). Fails closed to an empty list on a
// missing VK or ledger error — never leaks the full upstream list.
//
// Leak-proof by construction: the returned response can only contain owned rows
// and the caller's own ids; no cursor is emitted, so nothing can encode a foreign
// batch id. See the DEFERRED LIMITATION note at the top of the file for why deep
// multi-page owner pagination is not done here.
func (p *GovernancePlugin) filterBatchListForCaller(
	...
	vk, ok := p.store.GetVirtualKey(ctx, vkValue)
	if !ok || vk == nil {
		// Unknown VK: fail closed to an empty, cursor-less, non-raw list.
		emptyBatchList(list)
		return
	}
	if p.isBatchOwnershipAdmin(vk.ID) {
	...
	// Clamp the pre-allocation capacity so an absurd caller ?limit/page_size can
	// never drive a giant allocation. The filtered set is bounded by this single
	...
	// first_id/last_id: the caller's OWN ids (or nil). No next-page cursor.
	if len(filtered) == 0 {
		list.FirstID = nil
		list.LastID = nil
	} else {
	...
	list.HasMore = false
	list.NextCursor = nil
	// Suppress raw passthrough: anthropic.go / openai.go / genai.go list
	// converters echo ExtraFields.RawResponse verbatim when present (the full
	// upstream list), bypassing the filtered Data. Clearing it forces every
	// dialect converter down its rebuild-from-Data path.
